...
const transform: AxiosTransform = {
  ...
  try {
    ...
    if (response) {
      ...
      // 使用原有的 checkStatus 处理特殊状态码（如401跳转登录）
      if (checkStatus && typeof checkStatus === 'function') {
        const responseMsg = response.data?.message || message || errorMessage
        checkStatus(status, responseMsg, showErrorMessage, response)
      } else {
    ...
    // 其他未知错误
    const unknownError = message || '未知错误'
