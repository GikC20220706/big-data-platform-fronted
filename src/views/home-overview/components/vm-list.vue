...
import { transformExecutionData, calculateDuration } from '@/utils/data-transform'
import { mapStatus } from '@/utils/status-mapping'
...
function queryVmlistData(loading = false) {
  if (!loading) {
  ...
  http.request({
    method: 'get',
    url: '/api/v1/scheduler/executions',
    params: {
      page: paginationInfo.value.page + 1, // 后端从1开始
      page_size: paginationInfo.value.pageSize,
      keyword: keyWord.value || undefined,
      execution_date: dayjs().format('YYYY-MM-DD'), // 只显示今天的执行
      sort_by: 'start_time',
      sort_order: 'desc'
    }
  }).then((response: any) => {
    if (response.success) {
      // 数据格式转换
      const executions = response.data?.items || response.data?.executions || []
      tableData.value = executions.map((item: any) => transformExecutionData(item))
...
function reRunWorkFlowDataEvent(data: any) {
  http.request({
    method: 'post',
    url: `/api/v1/scheduler/executions/${data.workflowInstanceId}/rerun`,
    data: {
